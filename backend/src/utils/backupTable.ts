...
export function createDBBackupStreaming(): Promise<string | null> {
  return new Promise((resolve, reject) => {
    ...
    if (!DB_PASSWORD || !DB_USER || !DB_NAME || !DB_HOST) {
      ...
      return reject(
        new Error("Missing required environment variables for backup")
      );
    }
    ...
    try {
      ...
      return reject(
        new Error(`Failed to ensure backup directory exists: ${dirErr.message}`)
      );
    }
    ...
    logger.info(
      `Creating streaming compressed backup to: ${finalBackupFilePath}`
    );
    ...
    pgDump.on("close", async (code: number | null) => {
      ...
      logger.info(
        `Streaming compressed backup completed. Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`
      );
...
export function cleanOldBackups(): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    logger.info(
      `Cleaning old backups in ${backupDirPath}, keeping ${MAX_BACKUPS} most recent`
    );
    ...
    fs.readdir(backupDirPath, (err, files) => {
      if (err) {
        if (err.code === "ENOENT") {
          logger.warn(
            `Backup directory ${backupDirPath} not found. Skipping cleanup.`
          );
          resolve();
          return;
        }
        logger.error(
          `Error reading backup directory ${backupDirPath}: ${err.message}`
        );
        reject(err);
        return;
      }
        ...
        const sqlFiles = files
          ...
          .map(file => {
            ...
            try {
              ...
              logger.warn(
                `Could not stat file ${filePath} during cleanup: ${statErr.message}. Skipping.`
              );
              return null;
            }
          })
          .filter(
            (f): f is { file: string; path: string; time: number } => f !== null
          )
          .sort((a, b) => a.time - b.time);
        if (sqlFiles.length > MAX_BACKUPS) {
          const filesToDelete = sqlFiles.slice(
            0,
            sqlFiles.length - MAX_BACKUPS
          );
          logger.info(
            `Found ${sqlFiles.length} backups, deleting ${filesToDelete.length} oldest ones.`
          );
          ...
          const deletePromises = filesToDelete.map(
            ({ file, path: filePath }) => {
              return fs.promises
                ...
                .catch(unlinkErr => {
                  logger.error(
                    `Error deleting old backup ${file}: ${unlinkErr.message}`
                  );
          ...
          logger.info(
            `Found ${sqlFiles.length} backups, no cleanup needed (max: ${MAX_BACKUPS}).`
          );
          resolve();
        }
  ...
  return new Promise<FileData[]>((resolve, reject) => {
    ...
    fs.readdir(backupDirPath, (err, files) => {
      if (err) {
        if (err.code === "ENOENT") {
          logger.warn(
            `Backup directory ${backupDirPath} not found. Returning empty list.`
          );
          resolve([]);
          return;
        }
        ...
        const backups: FileData[] = files
          ...
          .map(file => {
            ...
            try {
              ...
              logger.warn(
                `Could not stat file ${filePath} during list: ${statErr.message}. Skipping file.`
              );
              return null;
            }
          })
